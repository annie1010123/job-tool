...
| 7 | Turbopack 弄壞 pdfjs worker，PDF 上傳連續失敗 | 🔴 高 | 4 輪嘗試半天 | ✅ 先查 bundler 相容性 |
| 8 | styled-jsx 抽共用元件後原頁面樣式全毀 | 🟡 中 | 1 小時 | ✅ 共用元件改動驗所有使用端 |
| 9 | Vercel 美東 × DB 雪梨，切頁秒級延遲 | 🟡 中 | 半小時 | ✅ 開案時就對齊 region |
...
## 坑 7：Turbopack 弄壞 pdfjs worker（PDF 上傳連續失敗）

**怎麼發現的：**
使用者（我自己以外的測試角色）實測上傳失敗兩次——而我兩次都只跑了 typecheck 就回報「修好了」。

**根因：**
Turbopack 打包 `pdfjs-dist` 時破壞了 worker 檔案的路徑解析，`require.resolve` 被改寫成找不到的內部路徑。

**怎麼解的：**
修了 4 輪「讓 bundler 找到 worker」全部失敗後**換路**：`next.config.ts` 加 `serverExternalPackages: ["pdfjs-dist","pdf-parse"]`，讓 Turbopack 完全不碰這個套件，一次解決。

**學到什麼：**
1. 同一個方向連續失敗兩次，該換路而不是加倍下注——「修好路徑」和「讓它不需要路徑」是兩條路
2. **typecheck 通過不等於功能正常**。驗證要走使用者會走的那條路（實際上傳一個 PDF），這之後變成我的硬性流程

---

## 坑 8：styled-jsx 抽共用元件後，原頁面樣式全毀

**怎麼發現的：**
把練習彈框從 /interview 抽成共用元件給作戰室用，只測了新頁面就回報完成；使用者打開原頁面，彈框變成散落整頁的表單加一個巨大黑色圖示。

**根因：**
`<style jsx global>` 在 App Router 的樣式注入不穩定——同一個元件在新頁面正常、在原頁面完全沒注入 CSS。

**怎麼解的：**
放棄 styled-jsx，改用 `dangerouslySetInnerHTML` 輸出普通 `<style>` 標籤——行為在任何頁面都確定。

**學到什麼：**
**重構共用的東西，每一個使用端都要各自實測**。「另一頁用同一個元件而且正常」不是證據，是錯覺。

---

## 坑 9：頁面切換慢——兇手是地理，不是程式碼

**怎麼發現的：**
正式站切換頁面超過 1 秒，使用者要求所有操作壓進 0.5 秒。

**根因：**
Vercel serverless 函式預設跑在美國東岸，Supabase 在雪梨。每個頁面要做 3–5 次資料庫查詢（含登入 session 檢查），每次都付約 200ms 的跨太平洋來回——疊加起來 1–2 秒。

**怎麼解的：**
`vercel.json` 加 `"regions": ["syd1"]` 把函式釘到雪梨與資料庫同機房，單次查詢降到個位數 ms；再補齊缺漏的 `loading.tsx` 讓切頁有即時骨架回饋。

**學到什麼：**
延遲預算要算的是「函式 ↔ 資料庫」的距離，不只是「使用者 ↔ 伺服器」。多次查詢會放大單次延遲——N 次查詢 × 200ms 的痛，遠大於一次 130ms 的洲際跳。
